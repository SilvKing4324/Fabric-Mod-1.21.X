...
public class GeckoModel<T extends GeckoEntity> extends SinglePartEntityModel<T> {

    public static final EntityModelLayer GECKO = new EntityModelLayer(Identifier.of(SilvKingsMod.MOD_ID, "gecko"), "main");
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        ModelPartData FLegL_r1 = FrontLegL.addChild("FLegL_r1", ModelPartBuilder.create().uv(11, 12).cuboid(-0.1428F, -0.4441F, -0.6528F, 2.0F, 1.0F, 1.0F, new Dilation(0.0F)), ModelTransform.of(-0.2612F, -0.0259F, 0.0752F, 0.0573F, 0.2106F, 0.2679F));

        ModelPartData FrontLegR = Body.addChild("FrontLegR", ModelPartBuilder.create(), ModelTransform.pivot(-1.275F, -0.4804F, -2.5515F));

        ModelPartData FLegR_r1 = FrontLegR.addChild("FLegR_r1", ModelPartBuilder.create().uv(0, 13).cuboid(-1.0F, -0.5F, -0.5F, 2.0F, 1.0F, 1.0F, new Dilation(0.0F)), ModelTransform.of(-0.5F, 0.25F, -0.25F, 0.0573F, -0.2106F, -0.2679F));

        ModelPartData BackLegL = Body.addChild("BackLegL", ModelPartBuilder.create(), ModelTransform.pivot(1.2612F, -0.5957F, 0.7739F));

        ModelPartData BLegL_r1 = BackLegL.addChild("BLegL_r1", ModelPartBuilder.create().uv(11, 2).cuboid(-0.832F, -0.4122F, -0.4744F, 3.0F, 1.0F, 1.0F, new Dilation(0.0F)), ModelTransform.of(-0.0612F, -0.0043F, -0.0239F, -0.0883F, -0.3958F, 0.2794F));

        ModelPartData BackLegR = Body.addChild("BackLegR", ModelPartBuilder.create(), ModelTransform.pivot(-1.2612F, -0.5957F, 0.7739F));

        ModelPartData BLegR_r1 = BackLegR.addChild("BLegR_r1", ModelPartBuilder.create().uv(11, 0).cuboid(-2.0F, -1.0F, -1.0F, 3.0F, 1.0F, 1.0F, new Dilation(0.0F)), ModelTransform.of(0.2612F, 0.5957F, 0.4761F, -0.0883F, 0.3958F, -0.2794F));

        ModelPartData bb_main = modelPartData.addChild("bb_main", ModelPartBuilder.create(), ModelTransform.pivot(0.0F, 24.0F, 0.0F));

        ModelPartData camera_r1 = bb_main.addChild("camera_r1", ModelPartBuilder.create().uv(0, 0).cuboid(-2.0F, -2.0F, 0.0F, 2.0F, 2.0F, 2.0F, new Dilation(0.0F)), ModelTransform.of(0.0F, 0.0F, 0.0F, -2.9795F, -0.6308F, 3.1416F));
    ...
    @Override
    public void setAngles(GeckoEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.getPart().traverse().forEach(ModelPart::resetTransform);
        this.applyHeadRotation(netHeadYaw, headPitch);

        this.animateMovement(GeckoAnimations.GECKO_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        this.updateAnimation(entity.swimAnimationState, GeckoAnimations.GECKO_SWIM, ageInTicks, 1f);
        this.updateAnimation(entity.idleAnimationState, GeckoAnimations.GECKO_IDLE, ageInTicks, 1f);
        this.updateAnimation(entity.sleepAnimationState, GeckoAnimations.GECKO_SLEEP, ageInTicks, 1f);
    }
